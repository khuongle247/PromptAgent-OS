...
for (const f of filesToAudit) {
  ...
  try {
    ...
    if (result) {
      importedBy = result.split('\n').filter(x => x && !x.includes(f)).slice(0, 20);
    }
  } catch (e) {}
  ...
  try {
    ...
    const result = execSync(cmd, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] }).trim();
    if (result) {
      requiredBy = result.split('\n').filter(x => x && x !== f).slice(0, 20);
    }
  } catch (e) {}
  ...
  if (fs.existsSync(dir)) {
    try {
      ...
      alternatives = siblings.filter(s => 
        s.startsWith(pattern) && s !== basename && s.endsWith('.js')
      );
    } catch (e) {}
  }
