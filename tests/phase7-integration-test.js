...
async function runAll() {
  ...
  await bus.publish("agent-executed", { agent: "planner", taskId: "TASK-001", ok: true, duration: 50, timestamp: new Date().toISOString(), schemaVersion: "1.0", output: {}, validation: { valid: true, errors: [] }, context: {} });
  await bus.publish("artifact-written", { agent: "planner", taskId: "TASK-001", artifactPath: "test", schemaPath: "test", timestamp: new Date().toISOString(), schemaVersion: "1.0" });

  const auditLog = fs.existsSync(AUDIT_LOG) ? fs.readFileSync(AUDIT_LOG, "utf8").trim().split("\n").filter(Boolean) : [];
  ...
  assert(filterResult.every(r => r.eventType === "agent-executed"), "getAuditLog filter returns only matching events");
  ...
  assert(agentMetrics.totalExecutions >= 1, `totalExecutions >= 1 (got ${agentMetrics.totalExecutions})`);
  assert(agentMetrics.successfulExecutions >= 1, `successfulExecutions >= 1 (got ${agentMetrics.successfulExecutions})`);
  assert(agentMetrics.failedExecutions >= 0, `failedExecutions >= 0 (got ${agentMetrics.failedExecutions})`);

  // Check persisted file
  const agentFile = fs.existsSync(AGENT_METRICS) ? JSON.parse(fs.readFileSync(AGENT_METRICS, "utf8")) : null;
  assert(agentFile !== null, "agent-performance.json exists");
  assert(agentFile.totalExecutions === agentMetrics.totalExecutions, "Persisted agent metrics match in-memory");

  // ---- Test 4: MetricsEngine captures learning metrics ----
  console.log("\n--- Test 4: MetricsEngine learning metrics ---");
  await bus.publish("lesson-learned", { lessonId: "LESSON-001", type: "success-pattern", description: "Test lesson captured successfully.", references: ["TASK-001"], details: { taskTitle: "test" }, timestamp: new Date().toISOString(), schemaVersion: "1.0" });
  await bus.publish("memory-importance-updated", { memoryId: "MEM-001", oldImportance: 2, newImportance: 3, timestamp: new Date().toISOString(), schemaVersion: "1.0" });
  await bus.publish("reusable-pattern-identified", { patternId: "P-001", description: "Test pattern", count: 2, exampleReferences: [], timestamp: new Date().toISOString(), schemaVersion: "1.0" });

  const learningMetrics = MetricsEngine.getLearningMetrics();
  assert(learningMetrics.lessonsLearned >= 1, `lessonsLearned >= 1 (got ${learningMetrics.lessonsLearned})`);
  assert(learningMetrics.memoryUpdates >= 1, `memoryUpdates >= 1 (got ${learningMetrics.memoryUpdates})`);
  assert(learningMetrics.reusablePatterns >= 1, `reusablePatterns >= 1 (got ${learningMetrics.reusablePatterns})`);

  // Check persisted file
  const learningFile = fs.existsSync(LEARNING_METRICS) ? JSON.parse(fs.readFileSync(LEARNING_METRICS, "utf8")) : null;
  ...
  await bus.publish("healing-cycle-completed", { taskId: "TASK-002", passed: true, reason: "Fixed", attemptsMade: 1, timestamp: new Date().toISOString(), schemaVersion: "1.0" });

  const taskMetrics = MetricsEngine.getTaskMetrics();
  assert(taskMetrics.totalTasks >= 1, `totalTasks >= 1 (got ${taskMetrics.totalTasks})`);
  assert(taskMetrics.completedTasks >= 1, `completedTasks >= 1 (got ${taskMetrics.completedTasks})`);

  // Check persisted file
  const taskFile = fs.existsSync(TASK_METRICS) ? JSON.parse(fs.readFileSync(TASK_METRICS, "utf8")) : null;
  ...
  const beforeInvalidEvent = fs.existsSync(path.join(process.cwd(), "logs", "invalid-events.jsonl")) ? fs.readFileSync(path.join(process.cwd(), "logs", "invalid-events.jsonl"), "utf8") : "";
  await bus.publish("agent-executed", { agent: "planner", taskId: "BAD-ID", ok: true, output: {}, validation: { valid: true, errors: [] }, duration: 10, timestamp: new Date().toISOString(), schemaVersion: "1.0", context: {} });
  const afterInvalidEvent = fs.readFileSync(path.join(process.cwd(), "logs", "invalid-events.jsonl"), "utf8");
  assert(afterInvalidEvent.length > beforeInvalidEvent.length, "Invalid event was quarantined (log grew)");
