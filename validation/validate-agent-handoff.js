  ...
  if (contract.qualityGates && Array.isArray(contract.qualityGates)) {
    for (const gate of contract.qualityGates) {
      ...
      if (!result.passed) {
        addError(
          report,
          `Quality gate ${gate.id} failed: ${gate.description} - ${result.detail || ""}`
        );
        passed = false;
      }
    }
...
function evaluateQualityGate(gate, data, projectDir) {
  ...
  if (
    gate.id.startsWith("QG-PA") ||
    gate.id.startsWith("QG-AC") ||
    gate.id.startsWith("QG-CR") ||
    gate.id.startsWith("QG-RC") ||
    gate.id.startsWith("QG-RD") ||
    gate.id.startsWith("QG-DM")
  ) {
...
function evaluateGenericGate(gate, data) {
  ...
  switch (gate.id) {
    ...
    case "QG-PA-5":
      return {
        passed: data.tasks && data.tasks.every(t => /^[0-9]+[mhdw]$/.test(t.estimate || "")),
        detail: "Invalid estimates"
      };
    ...
    case "QG-AC-5":
      return {
        passed:
          ...
          data.taskAssignments.every(a => (a.implementationGuidance || "").length >= 20),
        detail: "Missing guidance"
      };

    // Coder → Reviewer gates
    case "QG-CR-2":
      return {
        passed: data.testResults && data.testResults.failed === 0,
        detail: `Failed tests: ${data.testResults?.failed || "unknown"}`
      };
