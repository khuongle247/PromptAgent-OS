...
function createEventValidationMiddleware(schemaRegistry, invalidEventLogPath) {
  ...
  return (eventType, payload, next) => {
    ...
    if (!schema) {
      console.warn(`EventValidationMiddleware: No schema found for eventType: ${eventType}, version: ${eventVersion}. Skipping validation.`);
      return next(eventType, payload); // Fail-safe: proceed without validation
    }

    const validate = ajv.getSchema(`${eventType}-${eventVersion}`);
    if (!validate) {
      console.error(`EventValidationMiddleware: Compiled schema not found in AJV for ${eventType}-${eventVersion}. This should not happen if schemaRegistry is initialized correctly.`);
      ...
      try {
        ...
        console.error(`EventValidationMiddleware: Error compiling schema for ${eventType}-${eventVersion}: ${e.message}`);
    ...
    if (isValid) {
      ...
      console.warn(`EventValidationMiddleware: Invalid event quarantined: ${eventType}. Errors logged to ${invalidEventLogPath}`);
