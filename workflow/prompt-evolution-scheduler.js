...
function loadConfig() {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      console.warn(
        `[PromptEvolutionScheduler] Failed to parse config at ${configPath}: ${err.message}`
      );
    }
  }
  ...
  if (typeof config.intervalSeconds === "number" && config.intervalSeconds > 0) {
    ...
    config.intervalMs = Math.round(
      (config.intervalHours || DEFAULT_CONFIG.intervalHours) * 3600 * 1000
    );
  }

  config.enabled = config.enabled !== false;
  config.healthThreshold = Number(config.healthThreshold) || DEFAULT_CONFIG.healthThreshold;
  config.retryRateThreshold =
    Number(config.retryRateThreshold) || DEFAULT_CONFIG.retryRateThreshold;
  config.failureRateThreshold =
    Number(config.failureRateThreshold) || DEFAULT_CONFIG.failureRateThreshold;
...
function shouldRunEvolution(config, healthReport, agentMetrics, intervalTriggered = false) {
  ...
  const retryTrigger =
    ...
    agentMetrics.retryCount / agentMetrics.totalExecutions > config.retryRateThreshold;
  const failureTrigger =
    agentMetrics.totalExecutions > 0 &&
    agentMetrics.failedExecutions / agentMetrics.totalExecutions > config.failureRateThreshold;
...
function summarizeEvaluation(config, healthReport, agentMetrics, intervalTriggered, didRun) {
  const info = [];
  info.push(
    `[PromptEvolutionScheduler] Health score: ${healthReport.score} (threshold ${config.healthThreshold})`
  );
  info.push(
    `[PromptEvolutionScheduler] Retry rate: ${agentMetrics.totalExecutions > 0 ? (agentMetrics.retryCount / agentMetrics.totalExecutions).toFixed(3) : 0} (threshold ${config.retryRateThreshold})`
  );
  info.push(
    `[PromptEvolutionScheduler] Failure rate: ${agentMetrics.totalExecutions > 0 ? (agentMetrics.failedExecutions / agentMetrics.totalExecutions).toFixed(3) : 0} (threshold ${config.failureRateThreshold})`
  );
  info.push(
    `[PromptEvolutionScheduler] Trigger reason: ${lastRunReason}${intervalTriggered ? " (interval elapsed)" : ""}`
  );
...
function runSchedulerOnce(options = {}) {
  ...
  const shouldRun =
    force || shouldRunEvolution(config, healthReport, agentMetrics, intervalTriggered);
  if (!shouldRun) {
  ...
  try {
    ...
    console.log(
      `[PromptEvolutionScheduler] Evolution result: autoPromote=${config.autoPromote} canAutoPromote=${result.canAutoPromote?.canPromote}`
    );
    return true;
  } catch (err) {
    console.error(
      "[PromptEvolutionScheduler] Evolution cycle failed:",
...
function startScheduler() {
  ...
  if (config.intervalMs <= 0) {
    console.warn(
      "[PromptEvolutionScheduler] Invalid interval configured; scheduler will not start."
    );
    initialized = true;
    return;
  }
